...
class TestRenderCleaningHtml:
    ...
    def test_includes_pwa_manifest_and_service_worker(self) -> None:
        html_text = render_cleaning_html(year=2026, bookings=[])
        assert 'rel="manifest" href="/airbnb/manifest.webmanifest?v=5"' in html_text
        assert "navigator.serviceWorker.register('/airbnb/sw.js?v=5'" in html_text
        ...
        assert 'href="/airbnb/icons/icon-192.png?v=5" sizes="192x192"' in html_text
        assert 'href="/airbnb/icons/icon-512.png?v=5" sizes="512x512"' in html_text
